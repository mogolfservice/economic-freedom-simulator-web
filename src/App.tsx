...
function App() {
  ...
  const u = ui[language]
  const assetTotalsLabel = {
    ko: { totalAssets: '총 자산', includedFiAssets: '은퇴 계산 포함 자산', usableFiAssets: '현재 사용가능 FI 자산' },
    en: { totalAssets: 'Total assets', includedFiAssets: 'Assets included for FI', usableFiAssets: 'Currently usable FI assets' },
    ja: { totalAssets: '総資産', includedFiAssets: '退職計算に含める資産', usableFiAssets: '現在利用可能なFI資産' },
  }[language]
  ...
  const configuredRetirementEndAge = useMemo(() => Math.max(100, ...state.livingExpenseBands.map((band) => band.endAge).filter(Number.isFinite)), [state.livingExpenseBands])
  ...
  const yearsToFi = useMemo(() => calculateYearsToRetirementReadiness({ currentAge: state.currentAge, monthlyContribution: state.monthlyContribution, monthlyRetirementExpense: adjustedMonthlyRetirementExpense, livingExpenseBands: adjustedLivingExpenseBands, annualReturn: realReturn, retirementYears: state.retirementYears, retirementEndAge: configuredRetirementEndAge, assets: state.assets, pensions: state.pensions, children: state.children, liabilities: state.liabilities }), [adjustedLivingExpenseBands, adjustedMonthlyRetirementExpense, configuredRetirementEndAge, realReturn, state.assets, state.children, state.currentAge, state.liabilities, state.monthlyContribution, state.pensions, state.retirementYears])
  ...
  const readinessHorizonYears = useMemo(() => calculateRetirementHorizonYears({ retirementAge, retirementYears: state.retirementYears, livingExpenseBands: adjustedLivingExpenseBands, retirementEndAge: configuredRetirementEndAge }), [adjustedLivingExpenseBands, configuredRetirementEndAge, retirementAge, state.retirementYears])
  const readiness = useMemo(() => calculateRetirementReadiness({ currentAge: state.currentAge, retirementAge, retirementYears: state.retirementYears, retirementEndAge: configuredRetirementEndAge, monthlyRetirementExpense: adjustedMonthlyRetirementExpense, livingExpenseBands: adjustedLivingExpenseBands, annualReturn: realReturn, assets: projectedRetirementAssets, pensions: state.pensions, children: state.children, liabilities: state.liabilities }), [adjustedLivingExpenseBands, adjustedMonthlyRetirementExpense, configuredRetirementEndAge, projectedRetirementAssets, realReturn, retirementAge, state.children, state.currentAge, state.liabilities, state.pensions, state.retirementYears])
  const sensitivity = useMemo(() => getSensitivityMatrix({ currentFiAssets: access.unlockedFiAssets, monthlyContribution: state.monthlyContribution, monthlyRetirementExpense: adjustedMonthlyRetirementExpense, livingExpenseBands: adjustedLivingExpenseBands, annualReturn: realReturn, safeWithdrawalRate: state.safeWithdrawalRate, startYear: START_YEAR, retirementYears: state.retirementYears, retirementEndAge: configuredRetirementEndAge, currentAge: state.currentAge, assets: state.assets, pensions: state.pensions, children: state.children, liabilities: state.liabilities }), [access.unlockedFiAssets, adjustedLivingExpenseBands, adjustedMonthlyRetirementExpense, configuredRetirementEndAge, realReturn, state.assets, state.children, state.currentAge, state.liabilities, state.monthlyContribution, state.pensions, state.retirementYears, state.safeWithdrawalRate])
  ...
  const decisionScenarios = useMemo(() => createDecisionScenarios({ currentAge: state.currentAge, monthlyContribution: state.monthlyContribution, monthlyRetirementExpense: adjustedMonthlyRetirementExpense, livingExpenseBands: adjustedLivingExpenseBands, annualReturn: realReturn, safeWithdrawalRate: state.safeWithdrawalRate, startYear: START_YEAR, retirementYears: state.retirementYears, retirementEndAge: configuredRetirementEndAge, assets: state.assets, pensions: state.pensions, children: state.children, liabilities: state.liabilities }), [adjustedLivingExpenseBands, adjustedMonthlyRetirementExpense, configuredRetirementEndAge, realReturn, state.assets, state.children, state.currentAge, state.liabilities, state.monthlyContribution, state.pensions, state.retirementYears, state.safeWithdrawalRate])
  ...
  return (
    <main className="app-shell">
      ...
      <section className="metrics-grid" aria-label={t.metricsLabel}>
        ...
        <MetricCard label={t.stability} value={readiness.success ? t.passed : t.caution} note={readiness.success ? u.reflectedPensionLockedAssets(readinessHorizonYears) : u.depletedAtAge(readiness.depletedAge)} tone={readiness.success ? 'good' : 'warn'} />
      </section>
